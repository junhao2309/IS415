...
```{r}
wp_functional[1:length(wp_functional$status_clean)] <- "Functional"
wp_nonfunctional[1:length(wp_nonfunctional$status_clean)] <- "Non-Functional"
```
...
Due to rounding, the graph shows Non.Functional local colocation as 1 but in fact it is less than one. Features of the category of interest (Functional Waterpoints) that have colocation quotients less than one are less likely to have category B (Non-functional) within their neighborhood. 
Since p-value is less than 0.05, we can say that the result is statistically significant and that Functional and Non-functional water points are dependent from each other. 

```{r}
pacman::p_load(spdep)
```

A bonus analysis that is taught in Spatial Weights in Lesson 6 is by using the spdep package.
