...
```{r}
pacman::p_load(tidyverse, sf, sfdep, tmap, httr, jsonlite, matrixStats, readr, readxl, GWmodel, SpatialML, rsample)
```
...
str_list <- str_split(Resale_train$remaining_lease, " ")

for (i in 1:length(str_list)) {
  if (length(unlist(str_list[i])) > 2) {
      year <- as.numeric(unlist(str_list[i])[1])
      month <- as.numeric(unlist(str_list[i])[3])
      Resale_train$remaining_lease[i] <- year + round(month/12, 2)
  }
  else {
    year <- as.numeric(unlist(str_list[i])[1])
    Resale_train$remaining_lease[i] <- year
  }
}
```
...
Resale_test <- Resale %>%
  select(1,3,4,5,6,7,10,11) %>%
  mutate(month = as.Date(paste0(month, "-01"), format ="%Y-%m-%d")) %>%
  filter(month == c("2023-01-01","2023-02-01"),
         flat_type == "5 ROOM")
```
...
```{r, eval = FALSE}
# Splits the string by year and month, using str_split from the stringr package
str_list <- str_split(Resale_test$remaining_lease, " ")

for (i in 1:length(str_list)) {
  if (length(unlist(str_list[i])) > 2) {
      year <- as.numeric(unlist(str_list[i])[1])
      month <- as.numeric(unlist(str_list[i])[3])
      Resale_test$remaining_lease[i] <- year + round(month/12, 2)
  }
  else {
    year <- as.numeric(unlist(str_list[i])[1])
    Resale_test$remaining_lease[i] <- year
  }
}
```
:::
...
for (i in 1:nrow(Resale_train)){
  temp_output <- geocode(Resale_train[i, 3], Resale_train[i, 4])
...
for (i in 1:nrow(Resale_test)){
  temp_output <- geocode(Resale_test[i, 3], Resale_test[i, 4])
...
```{r, eval = FALSE}
Resale_sf <- st_as_sf(Resale_train, 
...
```{r}
Resale_training <- st_read(dsn = "data/aspatial",
                     layer = "resale")
```
...
```{r}
Resale_test <- st_read(dsn = "data/aspatial",
                     layer = "resale_test")
```
:::
...
![](images/bw_adaptive_run.png)eturn(df1) }
...
training_resale <- 
  ...
  proximity(Resale_training, cbd_sf, "PROX_CBD") %>%
...
test_resale <- 
  # the columns will be truncated later on when viewing 
  # so we're limiting ourselves to two-character columns for ease of viewing between
  proximity(Resale_test, cbd_sf, "PROX_CBD") %>%
  proximity(.,Bus_stop, "PROX_BUS") %>%
  proximity(., communityclubs, "PROX_CLUBS") %>%
  proximity(., eldercare, "PROX_ELDERCARE") %>%
  proximity(., familyservices, "PROX_FAM") %>%
...
```{r, eval = FALSE}
st_write(training_resale, dsn = "data/aspatial/training_resale.shp")
```
...
```{r, eval = FALSE}
st_write(test_resale, dsn = "data/aspatial/test_resale.shp")
```
:::
...
```{r}
training_data <- st_read(dsn="data/aspatial",
                         layer = "training_resale")
test_data <- st_read(dsn = "data/aspatial",
                     layer = "test_resale")
```
...
We can see that we have forgotten to as.factor the categorical variable columns.

```{r}
training_data <- training_data %>%
  mutate(FLAT_TY = as.factor(FLAT_TY),
         STOREY = as.factor(STOREY))
```
...
training_data_nogeo <- training_data %>%
  st_drop_geometry()
corrplot::corrplot(cor(training_data_nogeo[, 7:28]), 
                   diag = FALSE, 
                   order = "AOE",
                   tl.pos = "td", 
                   tl.cex = 0.5, 
...
We can see several correlated variables but they are within acceptable range to be included in the regression.

# Building a non-spatial multiple linear regression

Since Area and Flat Type are most likely highly correlated, we will drop Flat Type.

```{r}
price_mlr <- lm(PRICE ~ STOREY + AREA_SQ + REMAINI + PROX_CB + PROX_CL + PROX_EL + PROX_FA + PROX_MR + PROX_HA + PROX_PH + PROX_PA + PROX_MA + PROX_SP + PROX_GO + NUM_KND + NUM_CHI +NUM_BUS+NUM_PRI + NUM_SEC,
                data = training_data)
summary(price_mlr)
...
bw_adaptive <- bw.gwr(PRICE ~ STOREY + AREA_SQ + REMAINI + PROX_CB + PROX_CL + PROX_EL + PROX_FA + PROX_MR + PROX_HA + PROX_PH + PROX_PA + PROX_MA + PROX_SP + PROX_GO + NUM_KND + NUM_CHI +NUM_BUS+NUM_PRI + NUM_SEC,
                data = train_data_sp,
                  approach="CV",
                  kernel="gaussian",
                  adaptive=TRUE,
...
![](images/bw_adaptive_run.png)

The smallest CV score is adaptive bandwidth: 372.
...
```{r, eval = FALSE}
gwr_adaptive <- gwr.basic(formula = PRICE ~ STOREY + AREA_SQ + REMAINI + PROX_CB + PROX_CL + PROX_EL + PROX_FA + PROX_MR + PROX_HA + PROX_PH + PROX_PA + PROX_MA + PROX_SP + PROX_GO + NUM_KND + NUM_CHI +NUM_BUS+NUM_PRI + NUM_SEC,
                          data = train_data_sp,
                          bw=bw_adaptive, 
                          kernel = 'gaussian', 
                          adaptive=TRUE,
...
```{r}
set.seed(1234)
rf <- ranger(PRICE ~ STOREY + AREA_SQ + REMAINI + PROX_CB + PROX_CL + PROX_EL + PROX_FA + PROX_MR + PROX_HA + PROX_PH + PROX_PA + PROX_MA + PROX_SP + PROX_GO + NUM_KND + NUM_CHI +NUM_BUS+NUM_PRI + NUM_SEC,
...
```{r, eval = FALSE}
gwRF_bw <- grf.bw(PRICE ~ STOREY + AREA_SQ + REMAINI + PROX_CB + PROX_CL + PROX_EL + PROX_FA + PROX_MR + PROX_HA + PROX_PH + PROX_PA + PROX_MA + PROX_SP + PROX_GO + NUM_KND + NUM_CHI +NUM_BUS+NUM_PRI + NUM_SEC,
                data = train_data_nogeo,
                kernel = "adaptive",
                coords = coords_train)
```
...
```{r, eval = FALSE}
set.seed(1234)
gwRF_adaptive <- grf(PRICE ~ STOREY + AREA_SQ + REMAINI + PROX_CB + PROX_CL + PROX_EL + PROX_FA + PROX_MR + PROX_HA + PROX_PH + PROX_PA + PROX_MA + PROX_SP + PROX_GO + NUM_KND + NUM_CHI +NUM_BUS+NUM_PRI + NUM_SEC,
