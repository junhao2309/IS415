...
The output of the code chunk below follows the same sequence as date where localmoran_test_stat\[\[1\]\] refers to "July 2021".

```{r}
month <- vaccination %>%
  filter(date == "July 2021")
vac_july.localMI <- cbind(month, localmoran_test_stat[[1]])
```

```{r}
tmap_mode("plot")
tm_shape(vac_july.localMI) +
  tm_fill(col = "Ii", 
          style = "pretty",
          palette = "RdBu",
...
Mistake made:: To delete later Computing Monte Carlo Moran's I With a similar code structure to the moran_test()

```{r, eval = FALSE}
moran_mctest <- function(mth){
  month <- vaccination %>%
    filter(date == mth)
  wm_q <- poly2nb(month,
                 queen = TRUE)
  rswm_q <- nb2listw(wm_q,
                     style = "W",
                     zero.policy = TRUE)
  result <- moran.mc(month$vaccination_rate, 
                    listw=rswm_q, 
                    nsim=999, 
                    zero.policy = TRUE, 
                    na.action=na.omit)
...
```{r, eval = FALSE}
set.seed(1234)
bperm <- lapply(date, FUN = moran_mctest)
bperm
```

Statistical conclusion (200words)

```{r, eval = FALSE}
for (i in c(1:12)){
  a<- summary(bperm[[i]]$res[1:999])
  b<- var(bperm[[i]]$res[1:999])
  print(paste(date[i]))
  print(a)
  print(b)
}
