...
By viewing geoDKI, row 243 and 244 is where all the NA resides at.
...
To read the files more efficiently, we will use the "for loop" function to read all the excel files into a data frame by using the read_excel() from the readxl package.
...
Hence, we have to add in these two subdistricts so that both dataframes will match when it is joint later on.
...
One example is using a sliderInput() to let the user their zoom level.
...
Here, we will be using the QUEEN mode and thus the argument: queen =TRUE.
